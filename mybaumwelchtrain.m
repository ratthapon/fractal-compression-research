function [guessTR,guessE,logliks] = mybaumwelchtrain(seqs,guessTR,logEmit,varargin)
...
% [checkE, numEmissions] = [numStates GMM.NComponents];
checkE = numStates;
numEmissions = size(logEmit,2);
guessE = rand(numStates,numEmissions);
if checkE ~= numStates
...
for iteration = 1:maxiter
    ...
    for count = 1:numSeqs
        ...
        if baumwelch   % Baum-Welch training
            ...
            [LL,iterTR]= myviterbi(seqs,guessTR,logEmit);
            loglik = loglik + LL;
            % w = warning('off');
            %warning(w);
