function [ CC, FBE, frames ] = frames_mfcc( frames, fs, Tw, Ts, alpha, window, R, M, N, L )
...
%   Author: Kamil Wojcicki, September 2011
    ...
    if( max(abs(frames))<=1 ), frames = frames * 2^15; end;
    ...
    frames = filter( [1 -alpha], 1, frames ); % fvtool( [1 -alpha], 1 );

    % Framing and windowing (frames as columns)
%     frames = vec2frames( frames, Nw, Ns, 'cols', window, false );
