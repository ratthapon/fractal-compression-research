function [ CC,FBE, OUTMAG, MAG, H, DCT] = mfcc3( speech, originSpeech, inFs, outFs, Tw, Ts, alpha, window, R, M, N, L )
...
%   Author: Kamil Wojcicki, September 2011
    ...
    if( nargin~= 12 ), help mfcc; return; end; 

    % Explode samples to the range of 16 bit shorts
    if( max(abs(speech))<=1 ), speech = speech * 2^15; end;
    if( max(abs(originSpeech))<=1 ), originSpeech = originSpeech * 2^15; end;

    Nw = round( 1E-3*Tw*outFs );    % frame duration (samples)
    Ns = round( 1E-3*Ts*outFs );    % frame shift (samples)
    ...
    hz2mel = @( hz )( 2595*log(1+hz/700) );     % Hertz to mel warping function 1127
    mel2hz = @( mel )( 700*exp(mel/2595)-700 ); % mel to Hertz warping function
    ...
    originSpeech = filter( [1 -alpha], 1, originSpeech );
    ...
    MAG = addHarToSpecFromCeps( originSpeech, MAG(1:K,:), inFs, outFs );

    % Triangular filterbank with uniformly spaced filters on mel scale
    H = trifbank( M, K, R, outFs, hz2mel, mel2hz ); % size of H is M x K 
