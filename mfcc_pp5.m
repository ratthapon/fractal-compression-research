function [ CC,FBE, OUTMAG, MAG, H, DCT] = mfcc_pp5( speech, fs, Tw, Ts, alpha, window, R, M, N, L, a)
...
%   Author: Kamil Wojcicki, September 2011
    ...
    K = Nw/2+1; %nfft/2+1;                % length of the unique part of the FFT 
    ...
    MAG = abs( fft(frames,Nw,1) ); 

    % Triangular filterbank with uniformly spaced filters on mel scale
    [H, f, c] = trifbank( M, K, R, fs, hz2mel, mel2hz ); % size of H is M x K 
%     a = 16000/fs;
    B = find((c(2:end-1)>(fs/2 * 1/a)), 1);
    if a == 2
%         MAG = upsample(MAG,a);
%         w = size(H, 2);
%         H = H(:,2:a:end);
%         H = [H zeros(size(H,1), w - size(H, 2))];
    end
    ...
    FBE(B:end,:) = 0;
    gamma = 0.9;
    for decay = 1:(size(FBE, 1) - B)+1
        FBE(B + decay - 1,:) = FBE(B - 1,:) * (gamma.^decay);
    end
    OUTMAG = MAG(1:K,:);
%     FBE = ones(size(H))*MAG(1:K,:); % skip filter
    ...
    % normalize
%     FBE = FBE - repmat(mean(FBE,2),1,size(FBE,2));
%     FBE(FBE<0) = eps;
%     FBE = FBE / norm(FBE);
%     CC =  log(FBE); % MY ASSUMTION
